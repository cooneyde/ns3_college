...
//	remoteHostPosition->Add(Vector(50,50,0));
	...
	NetDeviceContainer UEDevs[number_enbs];

	//Set up DataRadioBearer
	Ipv4InterfaceContainer ueIpIface[number_enbs];
	...
	for(int i = 0; i < number_enbs; i++)
	{
		...
		{
			lteHelper->ActivateDedicatedEpsBearer(UEDevs[i].Get(j), EpsBearer(EpsBearer::GBR_CONV_VOICE), EpcTft::Default());
	...
	for(uint32_t i=0; i<PeNBnodes.GetN();i++)
	{
		...
		cout << LtePeNBDev[i]->GetDlBandwidth() << "  " << LtePeNBDev[i]->GetDlEarfcn()<<endl;
