...
int main(int argc, char *argv[])
{
	...
	int number_MUEs = 1;
	int number_UEs_Per_eNB = 2;
	int number_enbs = number_PeNB + number_MeNB;
	int number_UEs = (number_enbs) * number_UEs_Per_eNB;
...
//	remoteHostPosition->Add(Vector(50,50,0));
	...
	for(int i = 0; i < number_enbs; i++)
	{
		...
		lteHelper->ActivateDedicatedEpsBearer(UEDevs[i].Get(0), EpsBearer(EpsBearer::GBR_CONV_VIDEO), EpcTft::Default());
	...
	for (int i=0; i<number_enbs;i++)
	{
		for (uint32_t j = 0; j < UEnodes[i].GetN (); j++)
		{
			...
			serverApps.Add (packetSinkHelper.Install (UEnodes[i].Get(j)));
			 ...
			 if (i+1 < number_enbs)
	         {
		        clientApps.Add (client.Install (UEnodes[i+1].Get(j)));
		     }
		     else
	         {
		        clientApps.Add (client.Install (UEnodes[i].Get(j)));
		     }
		}
	}
