...
int main(int argc, char *argv[])
{
	...
	int number_PeNB = 3;
	int number_MeNB = 1;
	int number_PUEs = 10;		//number of UEs per PeNB
	int number_MUEs = 20;
...
//	remoteHostPosition->Add(Vector(50,50,0));
	...
	/*for(int i=0;i<number_PeNB;i++)
	{
		...
		if (i-1>-1)
		{
			...
			PeNBposition[i].y= (MeNBposition.y + PeNBposition[i-1].y) * 0.5 + (1.5*PeNB_radius);
		}
	...
	PeNBposition[0].x = MeNBposition.x + PeNB_radius;
		PeNBposition[0].y = MeNBposition.y + PeNB_radius;

		for(int i=1;i<number_PeNB;i++)
		{
			if(i % 2 == 0)
			{
				PeNBposition[i].x = PeNBposition[i-1].x - PeNB_radius;
				PeNBposition[i].y = PeNBposition[i-1].y + 2*PeNB_radius;
			}
			else
			{
				PeNBposition[i].x = PeNBposition[i-1].x + 2*PeNB_radius;
				PeNBposition[i].y = PeNBposition[i-1].y;
			}
	...
	for(int i=0; i<number_enbs;i++)
	{
		if (i >= number_PeNB)
		{
			...
			UEmobility[i].SetMobilityModel ("ns3::RandomWalk2dMobilityModel",
						                    ...
						                    "Time", StringValue ("3.0s"),					//Delay defines how long until the UE changes direction and speed
	...
	NetDeviceContainer PeNBDevs = lteHelper->InstallPEnbDevice (PeNBnodes, MeNBnodes, false);
	...
	Simulator::Destroy();
	monitor->SerializeToXmlFile("results_no_icic.xml", true, true);
