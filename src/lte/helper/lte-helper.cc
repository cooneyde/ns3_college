...
NetDeviceContainer
LteHelper::InstallPEnbDevice (NodeContainer c, NodeContainer m, bool schedstat)
{
  NS_LOG_FUNCTION (this);
  Initialize ();  // will run DoInitialize () if necessary
  NetDeviceContainer devices;
  ...
  for (NodeContainer::Iterator i = c.Begin (); i != c.End (); ++i)
    {
	  NodeContainer::Iterator m_node = m.Begin();
	  Ptr<Node> MeNB = *m_node;
      Ptr<Node> node = *i;
      Ptr<NetDevice> device = InstallSinglePEnbDevice (node, MeNB, schedstat);
      devices.Add (device);
    }
  return devices;
}
...
Ptr<NetDevice>
LteHelper::InstallSinglePEnbDevice (Ptr<Node> n, Ptr<Node> MeNB, bool schedstat)
{

	// Installs ENB device without connecting it to the epc but to a MeNB
  ...
  if(schedstat == true)		//Used to check if the ICIC protocols are being used here
  {

	  m_schedulerFactory.SetTypeId("ns3::PfFfPenbMacScheduler");
  }
  ...
  if (m_epcHelper != 0)
    {
      ...
      m_epcHelper->AddPEnb(n, MeNB, dev, dev->GetCellId());
