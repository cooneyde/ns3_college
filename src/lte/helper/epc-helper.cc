...
void
EpcHelper::AddPEnb(Ptr<Node> Penb, Ptr<Node> Menb, Ptr<NetDevice> ltePenbNetDevice, uint16_t cellId)
{
	NS_LOG_FUNCTION (this << Penb << ltePenbNetDevice << cellId);

	  NS_ASSERT (Penb == ltePenbNetDevice->GetNode ());

	  // add an IPv4 stack to the previously created eNB
	  InternetStackHelper internet;
	  internet.Install (Penb);
	  NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB after node creation: " << Penb->GetObject<Ipv4> ()->GetNInterfaces ());

	  // create a point to point link between the new eNB and the Menb with
	  // the corresponding new NetDevices on each side
	  NodeContainer PenbMenbNodes;
	  PenbMenbNodes.Add (Menb);
	  PenbMenbNodes.Add (Penb);
	  ...
	  NetDeviceContainer PenbMenbDevices = p2ph.Install (Penb, Menb);
	  NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB after installing p2p dev: " << Penb->GetObject<Ipv4> ()->GetNInterfaces ());
	  Ptr<NetDevice> PenbDev = PenbMenbDevices.Get (0);	//Gets the netdevice associated with the Penb
	  Ptr<NetDevice> MenbDev = PenbMenbDevices.Get (1); //Gets the netdevice associated with the Menb
	  m_s1uIpv4AddressHelper.NewNetwork ();
	  Ipv4InterfaceContainer PenbMenbIpIfaces = m_s1uIpv4AddressHelper.Assign (PenbMenbDevices);
	  NS_LOG_LOGIC ("number of Ipv4 ifaces of the eNB after assigning Ipv4 addr to S1 dev: " << Penb->GetObject<Ipv4> ()->GetNInterfaces ());

	  Ipv4Address PenbAddress = PenbMenbIpIfaces.GetAddress (0);
	  Ipv4Address MenbAddress = PenbMenbIpIfaces.GetAddress (1);

	  // create S1-U socket for the Penb
	  Ptr<Socket> PenbS1uSocket = Socket::CreateSocket (Penb, TypeId::LookupByName ("ns3::UdpSocketFactory"));
	  int retval = PenbS1uSocket->Bind (InetSocketAddress (PenbAddress, m_gtpuUdpPort));
	  NS_ASSERT (retval == 0);
	  ...
	  Ptr<Socket> PenbLteSocket = Socket::CreateSocket (Penb, TypeId::LookupByName ("ns3::PacketSocketFactory"));
	  PacketSocketAddress PenbLteSocketBindAddress;
	  PenbLteSocketBindAddress.SetSingleDevice (ltePenbNetDevice->GetIfIndex ());
	  PenbLteSocketBindAddress.SetProtocol (Ipv4L3Protocol::PROT_NUMBER);
	  retval = PenbLteSocket->Bind (PenbLteSocketBindAddress);
	  NS_ASSERT (retval == 0);
	  PacketSocketAddress PenbLteSocketConnectAddress;
	  PenbLteSocketConnectAddress.SetPhysicalAddress (Mac48Address::GetBroadcast ());
	  PenbLteSocketConnectAddress.SetSingleDevice (ltePenbNetDevice->GetIfIndex ());
	  PenbLteSocketConnectAddress.SetProtocol (Ipv4L3Protocol::PROT_NUMBER);
	  retval = PenbLteSocket->Connect (PenbLteSocketConnectAddress);
	  NS_ASSERT (retval == 0);


	  NS_LOG_INFO ("create MenbPenbApplication");
	  Ptr<EpcEnbApplication> PenbApp = CreateObject<EpcEnbApplication> (PenbLteSocket, PenbS1uSocket, PenbAddress, MenbAddress, cellId);
	  Penb->AddApplication (PenbApp);
	  NS_ASSERT (Penb->GetNApplications () == 1);
	  NS_ASSERT_MSG (Penb->GetApplication (0)->GetObject<EpcEnbApplication> () != 0, "cannot retrieve EpcEnbApplication");
	  NS_LOG_LOGIC ("enb: " << Penb << ", enb->GetApplication (0): " << Penb->GetApplication (0));


	  NS_LOG_INFO ("Create EpcX2 entity");
	  Ptr<EpcX2> x2 = CreateObject<EpcX2> ();
	  Penb->AggregateObject (x2);

	  NS_LOG_INFO ("connect S1-AP interface");
	  m_mme->AddEnb (cellId, PenbAddress, PenbApp->GetS1apSapEnb ());
	  m_sgwPgwApp->AddEnb (cellId, PenbAddress, MenbAddress);
	  PenbApp->SetS1apSapMme (m_mme->GetS1apSapMme ());
