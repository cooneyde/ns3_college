...
struct BwPart_s
{
  uint8_t   m_bwPartIndex;		//Bandwidth part for which cqi is being reported
  uint8_t   m_sb;				//preferred subband in current bandwidth
  uint8_t   m_cqi;				//CQI on conditioned subband
};
...
{
  uint8_t   m_sbPmi;				//PMI conditioned on current subband
  std::vector <uint8_t> m_sbCqi;	//CQI for up to two codewords conditioned on current subband
};
...
struct UeSelected_s
{
  std::vector <uint8_t> m_sbList;		// Preferred M subbands reported by the UE
  uint8_t   m_sbPmi;					// PMI conditioned on preferred subbands
  std::vector <uint8_t> m_sbCqi;		// CQI for up to two codewords conditioned on preferred subbands
};
...
{
  struct UeSelected_s           m_ueSelected;						// Subband measurement results for ue subbands
  std::vector <struct HigherLayerSelected_s> m_higherLayerSelected; // subband measurement results for higher layer selected subands
  struct BwPart_s               m_bwPart;							// indicates current cqi for best subband in current bandwidth
};
