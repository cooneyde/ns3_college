...
public:
	...
	virtual void RrcCellConfigurationUpdateInd(cellConfig params);
...
void EnbRrcMemberLteEnbCmacSapUser::RrcCellConfigurationUpdateInd(
		cellConfig params) {
	m_rrc->DoRrcCellConfigurationUpdateInd(params); //Do something
}
...
UeManager::UeManager(Ptr<LteEnbRrc> rrc, uint16_t rnti, State s) :
		m_lastAllocatedDrbid(0), m_rnti(rnti), m_imsi(0), m_lastRrcTransactionIdentifier(
				...
				false), m_sourceX2apId(0), m_sourceCellId(0), m_needTransmissionModeConfiguration(
				false) {
...
void UeManager::DoInitialize() {
	...
	case INITIAL_RANDOM_ACCESS:
		m_connectionTimeout = Simulator::Schedule(
				m_rrc->m_connectionTimeoutDuration,
				&LteEnbRrc::ConnectionTimeout, m_rrc, m_rnti);
		break;
...
TypeId UeManager::GetTypeId(void) {
	static TypeId tid =
			TypeId("ns3::UeManager").SetParent<Object>().AddConstructor<
					...
					"Srb0", "SignalingRadioBearerInfo for SRB0", PointerValue(),
					...
					MakeTraceSourceAccessor(
							&UeManager::m_stateTransitionTrace));
	return tid;
}
...
void UeManager::PrepareHandover(uint16_t cellId) {
	...
	case CONNECTED_NORMALLY: {
		...
		NS_LOG_LOGIC ("oldEnbUeX2apId = " << params.oldEnbUeX2apId); NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId); NS_LOG_LOGIC ("targetCellId = " << params.targetCellId); NS_LOG_LOGIC ("mmeUeS1apId = " << params.mmeUeS1apId); NS_LOG_LOGIC ("rrcContext   = " << params.rrcContext);
...
void UeManager::SendData(uint8_t bid, Ptr<Packet> p) {
	...
	case HANDOVER_PATH_SWITCH: {
		...
		LtePdcpSapProvider* pdcpSapProvider =
				GetDataRadioBearerInfo(drbid)->m_pdcp->GetLtePdcpSapProvider();
...
void UeManager::RecvMeasurementReport(LteRrcSap::MeasurementReport msg) {
	NS_LOG_FUNCTION (this); NS_LOG_LOGIC ("measId " << (uint16_t) msg.measResults.measId
			<< " haveMeasResultNeighCells " << msg.measResults.haveMeasResultNeighCells
			<< " measResultListEutra " << msg.measResults.measResultListEutra.size ()); NS_LOG_LOGIC ("serving cellId " << m_rrc->m_cellId
	...
	NS_ASSERT_MSG(
			(msg.measResults.measId == 1) || (msg.measResults.measId == 2),
	...
	if (msg.measResults.measId == 1) {
		...
		if (m_servingCellMeasures->m_rsrq
				<= m_rrc->m_servingCellHandoverThreshold) {
			...
			for (std::map<uint16_t, Ptr<UeMeasure> >::iterator it =
					m_neighbourCellMeasures.begin();
					it != m_neighbourCellMeasures.end(); ++it) {
				if (it->second->m_rsrq > bestNeighbourRsrq) {
					Ptr<NeighbourRelation> neighbourRelation =
							m_rrc->m_neighbourRelationTable[it->second->m_cellId];
			...
			if (bestNeighbour) {
				...
				if ((bestNeighbour->m_rsrq - m_servingCellMeasures->m_rsrq
						>= m_rrc->m_neighbourCellHandoverOffset)
						&& (m_state == CONNECTED_NORMALLY)) {
					NS_LOG_LOGIC ("Trigger Handover to cellId " << targetCellId); NS_LOG_LOGIC ("target cell RSRQ " << (uint16_t) bestNeighbour->m_rsrq); NS_LOG_LOGIC ("serving cell RSRQ " << (uint16_t) m_servingCellMeasures->m_rsrq);
	...
	/// Event A4 (Neighbour becomes better than threshold)
		...
		if (msg.measResults.haveMeasResultNeighCells
			...
			for (std::list<LteRrcSap::MeasResultEutra>::iterator it =
				...
				if (m_rrc->m_neighbourRelationTable.find(it->physCellId)
						!= m_rrc->m_neighbourRelationTable.end()) {
					// Update neighbour info
					Ptr<NeighbourRelation> neighbourRelation =
							m_rrc->m_neighbourRelationTable[it->physCellId];
					NS_ASSERT_MSG(
				...
				{
					Ptr<NeighbourRelation> neighbourRelation = CreateObject<
							NeighbourRelation>();
					neighbourRelation->m_physCellId = it->physCellId;
				...
				if (m_neighbourCellMeasures.find(it->physCellId)
						!= m_neighbourCellMeasures.end()) {
					neighbourCellMeasures =
							m_neighbourCellMeasures[it->physCellId];
...
uint16_t UeManager::GetRnti(void) {
	return m_rnti;
}

uint64_t UeManager::GetImsi(void) {
	return m_imsi;
}

uint16_t UeManager::GetSrsConfigurationIndex(void) {
...
UeManager::State UeManager::GetState(void) {
	return m_state;
}
...
LteRrcSap::MeasConfig UeManager::BuildMeasConfig() {
	...
	reportConfigA2.reportConfigEutra.triggerType =
			LteRrcSap::ReportConfigEutra::EVENT;
	reportConfigA2.reportConfigEutra.eventId =
			LteRrcSap::ReportConfigEutra::EVENT_A2;
	reportConfigA2.reportConfigEutra.threshold1.choice =
			LteRrcSap::ThresholdEutra::THRESHOLD_RSRQ;
	reportConfigA2.reportConfigEutra.threshold1.range =
			m_rrc->m_eventA2Threshold;
	reportConfigA2.reportConfigEutra.hysteresis = 0;
	reportConfigA2.reportConfigEutra.timeToTrigger = 0;
	reportConfigA2.reportConfigEutra.triggerQuantity =
			LteRrcSap::ReportConfigEutra::RSRQ;
	reportConfigA2.reportConfigEutra.reportQuantity =
	...
	reportConfigA2.reportConfigEutra.reportInterval =
	...
	reportConfigA4.reportConfigEutra.triggerType =
			LteRrcSap::ReportConfigEutra::EVENT;
	reportConfigA4.reportConfigEutra.eventId =
			LteRrcSap::ReportConfigEutra::EVENT_A4;
	reportConfigA4.reportConfigEutra.threshold1.choice =
			LteRrcSap::ThresholdEutra::THRESHOLD_RSRQ;
	reportConfigA4.reportConfigEutra.threshold1.range =
			m_rrc->m_eventA4Threshold;
	reportConfigA4.reportConfigEutra.hysteresis = 0;
	reportConfigA4.reportConfigEutra.timeToTrigger = 0;
	reportConfigA4.reportConfigEutra.triggerQuantity =
			LteRrcSap::ReportConfigEutra::RSRQ;
	reportConfigA4.reportConfigEutra.reportQuantity =
			LteRrcSap::ReportConfigEutra::SAME_AS_TRIGGER_QUANTITY;
	reportConfigA4.reportConfigEutra.maxReportCells = LteRrcSap::MaxReportCells;
	reportConfigA4.reportConfigEutra.reportInterval =
			LteRrcSap::ReportConfigEutra::MS480;
...
LteEnbRrc::LteEnbRrc() :
		m_x2SapProvider(0), m_cmacSapProvider(0), m_rrcSapUser(0), m_macSapProvider(
				0), m_s1SapProvider(0), m_cphySapProvider(0), m_configured(
				false), m_lastAllocatedRnti(0), m_srsCurrentPeriodicityId(0), m_lastAllocatedConfigurationIndex(
				0),
...
TypeId LteEnbRrc::GetTypeId(void) {
	...
	static TypeId tid =
			TypeId("ns3::LteEnbRrc").SetParent<Object>().AddConstructor<
					...
					MakeEnumChecker(RLC_SM_ALWAYS, "RlcSmAlways", RLC_UM_ALWAYS,
							...
							PER_BASED, "PacketErrorRateBased")).AddAttribute(
					...
					MakeTimeChecker()).AddAttribute("SrsPeriodicity",
					...
					MakeTimeChecker()).AddAttribute("AdmitHandoverRequest",
					...
					MakeBooleanChecker()).AddAttribute("EventA2Threshold",
					...
					MakeUintegerAccessor(
							&LteEnbRrc::m_servingCellHandoverThreshold),
					MakeUintegerChecker<uint8_t>()).AddAttribute(
					"NeighbourCellHandoverOffset",
					"Minimum offset between serving and best neighbour cell to trigger the Handover",
					UintegerValue(1),
					MakeUintegerAccessor(
							&LteEnbRrc::m_neighbourCellHandoverOffset),
					MakeUintegerChecker<uint8_t>()).AddTraceSource(
					"NewUeContext",
					"trace fired upon creation of a new UE context",
					MakeTraceSourceAccessor(&LteEnbRrc::m_newUeContextTrace)).AddTraceSource(
					"ConnectionEstablished",
					"trace fired upon successful RRC connection establishment",
					MakeTraceSourceAccessor(
							&LteEnbRrc::m_connectionEstablishedTrace)).AddTraceSource(
					"ConnectionReconfiguration",
					"trace fired upon RRC connection reconfiguration",
					MakeTraceSourceAccessor(
							&LteEnbRrc::m_connectionReconfigurationTrace)).AddTraceSource(
					"HandoverStart",
					"trace fired upon start of a handover procedure",
					MakeTraceSourceAccessor(&LteEnbRrc::m_handoverStartTrace)).AddTraceSource(
					"HandoverEndOk",
					"trace fired upon successful termination of a handover procedure",
					MakeTraceSourceAccessor(&LteEnbRrc::m_handoverEndOkTrace)).AddTraceSource(
...
void LteEnbRrc::setNeighbourNodes(std::vector<uint16_t> neighbours) {
	m_neighbourNodes = neighbours;
}
...
void LteEnbRrc::SendHandoverRequest(uint16_t rnti, uint16_t cellId) {
	NS_LOG_FUNCTION (this << rnti << cellId); NS_LOG_LOGIC ("Request to send HANDOVER REQUEST");
	NS_ASSERT(m_configured);
...
void LteEnbRrc::DoRecvHandoverRequest(EpcX2SapUser::HandoverRequestParams req) {
	...
	NS_LOG_LOGIC ("oldEnbUeX2apId = " << req.oldEnbUeX2apId); NS_LOG_LOGIC ("sourceCellId = " << req.sourceCellId); NS_LOG_LOGIC ("targetCellId = " << req.targetCellId); NS_LOG_LOGIC ("mmeUeS1apId = " << req.mmeUeS1apId);
	...
	NS_LOG_LOGIC ("oldEnbUeX2apId = " << ackParams.oldEnbUeX2apId); NS_LOG_LOGIC ("newEnbUeX2apId = " << ackParams.newEnbUeX2apId); NS_LOG_LOGIC ("sourceCellId = " << ackParams.sourceCellId); NS_LOG_LOGIC ("targetCellId = " << ackParams.targetCellId);
...
void LteEnbRrc::DoRecvHandoverRequestAck(
	...
	NS_LOG_LOGIC ("oldEnbUeX2apId = " << params.oldEnbUeX2apId); NS_LOG_LOGIC ("newEnbUeX2apId = " << params.newEnbUeX2apId); NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId); NS_LOG_LOGIC ("targetCellId = " << params.targetCellId);
...
void LteEnbRrc::DoRecvHandoverPreparationFailure(
	...
	NS_LOG_LOGIC ("oldEnbUeX2apId = " << params.oldEnbUeX2apId); NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId); NS_LOG_LOGIC ("targetCellId = " << params.targetCellId); NS_LOG_LOGIC ("cause = " << params.cause); NS_LOG_LOGIC ("criticalityDiagnostics = " << params.criticalityDiagnostics);
...
void LteEnbRrc::DoRecvSnStatusTransfer(
	...
	NS_LOG_LOGIC ("oldEnbUeX2apId = " << params.oldEnbUeX2apId); NS_LOG_LOGIC ("newEnbUeX2apId = " << params.newEnbUeX2apId); NS_LOG_LOGIC ("erabsSubjectToStatusTransferList size = " << params.erabsSubjectToStatusTransferList.size ());
...
void LteEnbRrc::DoRecvUeContextRelease(
	...
	NS_LOG_LOGIC ("oldEnbUeX2apId = " << params.oldEnbUeX2apId); NS_LOG_LOGIC ("newEnbUeX2apId = " << params.newEnbUeX2apId);
...
void LteEnbRrc::DoRecvLoadInformation(EpcX2SapUser::LoadInformationParams params) {
	NS_LOG_FUNCTION (this);
	...
	cii = params.cellInformationList.back();
	m_currentUlHighInterferenceInformationList.ulHighInterferenceIndicationList =
			cii.ulHighInterferenceInformationList.back().ulHighInterferenceIndicationList;

	//Send HII up to the scheduler
	LteEnbCmacSapProvider::cellConfig cellConfigParams;
	cellConfigParams.HighInterferenceInformationList =
			m_currentUlHighInterferenceInformationList.ulHighInterferenceIndicationList;
	m_cmacSapProvider->CellUpdateConfigurationReq(cellConfigParams);
}
...
void LteEnbRrc::DoRecvUeData(EpcX2SapUser::UeDataParams params) {
	...
	NS_LOG_LOGIC ("Recv UE DATA FORWARDING through X2 interface"); NS_LOG_LOGIC ("sourceCellId = " << params.sourceCellId); NS_LOG_LOGIC ("targetCellId = " << params.targetCellId); NS_LOG_LOGIC ("gtpTeid = " << params.gtpTeid); NS_LOG_LOGIC ("ueData = " << params.ueData); NS_LOG_LOGIC ("ueData size = " << params.ueData->GetSize ());
...
void LteEnbRrc::DoRrcCellConfigurationUpdateInd(
		LteEnbCmacSapUser::cellConfig cmacParams) {
	NS_LOG_FUNCTION(this);
	m_currentUlHighInterferenceInformationList.ulHighInterferenceIndicationList =
			cmacParams.HighInterferenceInformationList;

	int size_vect = m_neighbourNodes.size();
	NS_LOG_LOGIC("Number of neighbour nodes = " << size_vect);

	for (int i = 0; i < size_vect - 1; i++) {
		uint16_t targetCellId = m_neighbourNodes[i];
		NS_LOG_LOGIC("Source Cell ID " << m_cellId << " Target cell id:" << targetCellId);
		EpcX2Sap::CellInformationItem cii;
		cii.sourceCellId = m_cellId;
		cii.ulHighInterferenceInformationList.resize(1);
		cii.ulHighInterferenceInformationList[0].ulHighInterferenceIndicationList =
				m_currentUlHighInterferenceInformationList.ulHighInterferenceIndicationList;
...
void LteEnbRrc::AddX2Neighbour(uint16_t cellId) {
	...
	NS_ASSERT_MSG(
			m_neighbourRelationTable.find (cellId) == m_neighbourRelationTable.end (),
			"There is already an entry in the Neighbour Relation Table for cellId " << cellId);

	Ptr<NeighbourRelation> neighbourRelation =
			CreateObject<NeighbourRelation>();
