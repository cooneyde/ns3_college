...
public:
   ...
   */
  virtual ~LteEnbRrc ();
  //Interference load information primitives

  std::vector<EpcX2Sap::UlInterferenceOverloadIndicationItem> m_currentUlInterferenceOverloadIndicationList;
  EpcX2Sap::UlHighInterferenceInformationItem m_currentUlHighInterferenceInformationList;
...
public:
  ...
  /*
   * Set the neighbouring nodes of this enb
   */
  void setNeighbourNodes(std::vector<uint16_t> neighbours);
...
private:
  ...
  void DoRrcCellConfigurationUpdateInd(LteEnbCmacSapUser::cellConfig cmacParams);
...
private:
  ...
  std::vector<uint16_t>m_neighbourNodes;
