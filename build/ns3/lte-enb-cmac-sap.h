...
public:
  ...
  /**
    * \brief Parameters for defining the high interference information to the RRC ?
    * (Dermot Cooney)
   */
  struct cellConfig
  {
    std::vector<bool> HighInterferenceInformationList;

  };


  /*
   * Notify the MAC of a cell config update requested by the RRC (Dermot Cooney)
   */
  virtual void CellUpdateConfigurationReq(cellConfig params) = 0;
...
public:
  ...
  /*
   * Notify the RRC of a cell config update requested by the MAC (normally, by the scheduler)  (Dermot Cooney)
   */
  virtual void RrcCellConfigurationUpdateInd (cellConfig params) = 0;
